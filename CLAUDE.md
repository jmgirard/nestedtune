...
Nested cross-validation for the tidymodels ecosystem. Orchestrates the
outer loop and delegates inner tuning to `tune`.

The package and GitHub repository are `nestedtune` (D-001). The local
working directory is still `nestedcv`, which is cosmetic and carries no
weight.
