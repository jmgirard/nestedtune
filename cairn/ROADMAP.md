...
- Cover the two unexercised fold-failure branches (nothing makes `last_fit()` or `tune_grid()` itself raise) and move `finalize_workflow()` inside a guarded region so no path between the two `tryCatch`es can still abort a run — added 2026-07-26 — M03 review finding F3, scored 42 (below the action threshold)
