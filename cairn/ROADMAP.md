...
- Guard the vignette's `ranger` use so CRAN's noSuggests check flavor cannot fail it — `set_engine("ranger")` runs unconditionally while `ranger` is Suggests-only, unlike the roxygen (`@examplesIf`) and the tests (`skip_if_not_installed`); a `requireNamespace()` check plus `knitr::knit_exit()` is the cheap fix, and a local `devtools::check()` cannot detect the gap because ranger is installed — added 2026-07-26 — M06 review finding F3, scored 78 (below the action threshold)
