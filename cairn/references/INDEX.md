...
- code-inventory.md — synthesis; every top-level function in `R/` sorted into core / glue / resampling-layer / furniture / ambiguous, 106 entries (F001–F106) plus the backtick-quoted `[.nested_results`. Names the tune fact that removes each `glue` entry and what rsample would have to accept for each `resampling-layer` one, at tune 2.1.0 and rsample 1.3.2.
