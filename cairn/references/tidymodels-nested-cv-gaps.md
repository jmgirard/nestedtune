...
- **G6** — a ROADMAP candidate row, deliberately not committed to. The statistics are unsettled; shipping an interval here without oracle backing would violate the oracle convention in `DESIGN.md`. _(2026-07-31: primary sources now on the shelf — `bates2023.md` and `krstajic2014.md`. They sharpen the gap rather than close it; see the G6 candidate row in `ROADMAP.md`.)_ _(2026-07-31, second ingestion: three more sources reshape the gap. `bengio2004.md` gives the negative result in its strongest form — no **universally** unbiased variance estimator, and repeating the outer splits does not help, which forecloses the obvious workaround. `bayle2020.md` supplies what the row said did not exist: an asymptotically exact CI for k-fold test error, O(n) to compute, with a reference implementation. `austern2020.md` shows the V-fold variance reduction is a joint property of the algorithm and the data distribution, so no fixed divisor is right for everyone. The gap is therefore no longer "no method exists" but three specific blockers — loss stability is unestablished for a tuned procedure, the valid interval targets a different estimand, and the per-observation losses it needs are discarded at `R/nested-tune-grid.R:385-396`.)_ _(2026-07-31, third ingestion: six more sources, and the gap changes character again. **A fourth blocker, categorical rather than practical** — `luo2026.md` proves that any assumption-free valid black-box test of a *procedure's* risk has power bounded by α·(1 + ·)^{N/n}, and v-fold nesting runs at N/n = v/(v−1) ≈ 1.11, so an assumption-free interval for the nested estimand cannot exist at the sample ratio the design uses. It is no longer only that nobody has built one. **The stability blocker is now three blockers wearing one name** — `nachum2026.md` adds squared loss stability (weaker than loss stability, and *necessary* under low loss variance) while showing loss stability is not necessary in general; `luo2025.md` shows the closest studied analogue of the needed assumption cannot be certified from data without exhaustive search; and `bayle2026.md` shows `bayle2020.md`'s condition is a *ratio* r = nγ/σ² that fails for comparisons even when both algorithms satisfy it individually. Any future plan must name which notion it means. **One thing got better** — `bayle2026.md`'s Figure 3 measures a Lasso whose λ is chosen by an inner CV and finds it relatively stable as a single algorithm across four orders of magnitude of n, the first evidence the shelf has that a tuned procedure can satisfy the condition blocker (1) doubts. **And one route is closed rather than open** — `gauran2025.md` does produce a valid test and CI from an exhaustive nested design, but its tractability and its validity both rest on a ridge hat-matrix closed form, which the contract boundary excludes for arbitrary workflows. Finally, `tsamardinos2018.md` needs the same per-observation predictions blocker (3) names, so two unrelated methods now converge on that one missing input.)_
