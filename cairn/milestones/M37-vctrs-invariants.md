...
- [x] T7 Review return: shed the class when `vec_cbind()`'s name repair moves a
      ...
      object in either argument position. Regression test first.
...
Second review round, 2026-08-31, after the defect return the first round made
and the T6/T7 repair. `main` is still at `b26eb77` and has not moved since the
branch was cut, so no merge was needed. PR
[#46](https://github.com/tidymodels/nestedtune/pull/46). The first round's
evidence and its eleven findings are superseded by what follows; the return and
the two findings that caused it are in the work log.

### Acceptance criteria — fresh evidence

Measured on this branch at `47692bb` plus the two gate fixes below, by
`pkgload::load_all()` and by `testthat::test_file()` on
`tests/testthat/test-vctrs-compat.R` (131 passing, 0 failures, 0 skips).

- AC1 — all five forms measured directly, one at a time: `vec_slice(x, 1)`,
  `vec_rbind(x, x)`, `vec_c(x, x)`, `rbind(x, x)` and `rename(x, fold = id)`
  each return `tbl_df/tbl/data.frame`, and every one of the five names in
  `results_attributes()` reads `NULL` on each. The file's five blocks pass.
- AC2 — `vec_slice(x, c(2, 1, 3))` returns a `nested_results` whose
  `outer_label`, `grid` and `metrics` are `identical()` to the source's and
  whose `id` is `identical()` to `x$id[c(2, 1, 3)]`. Passing control green:
  that order differs from `x$id`, so handing back the source unchanged fails.
- AC3 — `vec_cbind(x, tibble(extra = 1:3))` and `bind_cols(x, tibble(extra =
  1:3))` both return `nested_results/tbl_df/tbl/data.frame`, 3 rows x 10
  columns, with `identical()` class vectors.
- AC4 — `vec_cast(x, as_tibble(vec_ptype(x)))` returns `tbl_df/tbl/data.frame`
  carrying the source's names; the reverse cast raises a condition inheriting
  `vctrs_error_cast` and not `vctrs_error_cast_lossy`.
- AC5 — all five ordered pairs return a zero-row data frame rather than
  raising, asserted pair by pair and measured pair by pair.
- AC6 — `man/nested_tune_grid.Rd` regenerates with no diff. The gap sentence
  `main` carries ("vctrs methods that would close them are not written yet") is
  gone; the four-doors paragraph, the argument-order paragraph T7 added and the
  `group_by()`/`rowwise()`/`as_tibble()` paragraph are all present. The AC6 test
  block passes on a `nested_results`. `NEWS.md` carries three entries naming
  `rename()`, `rbind()`, `vec_slice()`, `vec_rbind()`, `vec_c()`, `vec_cbind()`
  and the `vctrs` dependency.
- AC7 — `devtools::check()`: `Status: OK`, 0 errors, 0 warnings, 0 notes,
  3m 40.4s. `devtools::test()` after the formatting fix below:
  `FAIL 0 | WARN 0 | SKIP 0 | PASS 2098`.

### Consistency gate

`cairn_validate.py` exit 0, every check PASS; 18 advisory warnings, all the
standing `references staleness` ones, none touched by this milestone. No
`DESIGN.md` principle changed — the diff adds a Known issues entry only — so
`cairn_impact.py` was not run. Toolchain slot: `devtools::document()` produces
no diff, `NAMESPACE` and `man/` regenerate clean, README.md is not older than
...
CI on PR #46: `pkgdown`, `test-coverage` and both codecov checks pass;
`format-suggest` failed on three sites `air 0.11.0` wanted wrapped
(`R/nested-results.R` x2, `test-vctrs-compat.R` x1). `air format .` run at the
gate; the change is whitespace only and the suite re-run above is against it.

### Independent review — three lenses, findings and disposition

[S] blame-history: no new defect. It reproduced findings 6-10 below and
verified T6 and T7 repair the two the return named. [S] prior-review record:
two findings, both already listed below (R3 and R9); the archived reviews,
LESSONS and the RB/RR pair were read, and PR #46's own inline comments are all
bot formatting nits.

[O] diff-bug returned eleven ranked findings. R1 and R2 were reproduced by the
session directly before triage.

- R1 (confirmed) — D-035 was written inside the `<!-- Template:` comment block
  at `cairn/DECISIONS.md:1056`, so the milestone's only decision record for the
  T6/T7 repair rendered nowhere. This is the same defect `b26eb77` fixed for
  D-030 one commit before this branch was cut. **Fixed at the gate.**
- R2 (confirmed) — `vctrs::vec_cbind_frame_ptype(x)` returns a 3-row,
  0-column object classed `nested_results` carrying `grid`, `metrics`,
  `outer_label` and `nestedtune_template_rows = 3`; `print()` on it reports
  "Outer folds: 3 requested, 0 completed" and then errors `invalid argument
  type`. The comment above the branch says "no columns and no rows", "reaches
  no caller" and "`x[0]` gets the class taken off by `[`", and all three are
  false as written — `results_ptype()` puts the class back on and `[0]` drops
  columns, not rows. Reachable only through a vctrs function documented
  experimental and keyword-internal, which is the ground RR04 Q4 judged the
  severity negligible on; what is new is that the milestone's own Q5 decision
  says the fix "empties it of anything false", and the printed 3-fold report
  is not empty of it. Extends the first round's F8.
- R3 (confirmed) — `vctrs::vec_rbind(x)` and `vctrs::vec_c(x)` on a single
  argument return a bare tibble where `dplyr::bind_rows(x)` returns a
  `nested_results`, on a call that adds, removes and reorders no row. The
  `@return` paragraph this milestone wrote says "It is one rule, reached
  through four doors". No acceptance criterion reaches it. First round's F5,
  raised again by two lenses and undisposed at the first gate.
- R4 (confirmed) — `vec_cbind(x, tibble(splits = 1:3), .name_repair =
  "minimal")` keeps the class with two columns named `splits`; the comment says
  `vec_cbind()` can only "add, recycle and REPAIR NAMES". `bind_cols()` with
  the same argument answers identically, so the door parity AC3 asserts holds
  and the record column is still first and still holds its values.
- R5 (confirmed) — `vec_cast.nested_results.nested_results()` stamps the record
  from `x` rather than from `to`, alone among the file's rules. Measured on two
  objects from different runs; no reachable harm, `vec_restore()` re-deciding
  after it.
- R6 — `test-vctrs-compat.R` calls `tibble::` in eight blocks with no
  `skip_if_not_installed("tibble")` while D-034 put `tibble` in Suggests.
  First round's F6.
- R7 — `expect_no_record()` loops `results_attributes()` only, so neither
  private carrier is asserted absent on a shedding path. T7 added the second
  one, so the gap grew with the feature. Swept: no leak today. First round's F7.
- R8 — `rbind.nested_results()` templates on `args[[1]]` positionally and
  promotes every data-frame argument to a tibble, so the result's class depends
  on argument order. First round's F10.
- R9 — D-032's Consequences paragraph says combining with a bare tibble
  "refuses rather than quietly producing a table", which measurement
  contradicts. D-035 says so in its own Consequences paragraph; DECISIONS.md is
  append-only, so the annotation is the correction, and R1's fix is what makes
  it readable. First round's F9.
- R10 — `NEWS.md` does not name the argument-order rule T7 introduced, though
  the `@return` gained a paragraph for it.
- R11 — `names<-.nested_results()` runs the full sort-and-compare rule on every
  name assignment; cost only. First round's F11.
...
nothing here returns the milestone under the return floor on its own; R2 and R3
are the two where a maintainer could reasonably call a load-bearing defect,
both of them prose that measurement contradicts rather than behavior a
criterion asked for.
