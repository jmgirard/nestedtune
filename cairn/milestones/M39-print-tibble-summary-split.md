...
- 2026-09-01: the issue link was missing and the user caught it at the merge gate. M39 is what issue #34 asks for — topepo's request to mimic `tune:::print.tune_results()` in `print()` and move the current print method's logic and annotations into a `summary()` — and neither the branch nor the PR referenced it, though D-037 and M40's file both do. PR #48's body now opens `Closes #34` and the squash-merge message carries the same line, so merging closes the issue.
