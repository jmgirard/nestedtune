...
- [ ] T5: In `fixture_key()`'s failure path, name a positional deep argument by its position (`` `object` `` where named, `position 2` where not); extend the depth test with a request built the way `memoised()` builds one — the deep value positional — expecting the error to name that position. (M42 review O3.)
- [ ] T6: In the derived axis test, state one fact independently of `formals()`: expect `object` and `resamples` among the enumerated axes for both orchestrators, failing naming what is missing; and flag a registry entry with no matching formal, naming it. Plant both defects and record each red run. (M42 review O1, O9.)
- [ ] T7: Brace the `nest()` loop so `air format --check` is clean; correct the helper comment's figure to the sorted-argument depth the guard measures (30 against 40, 2026-09-01) and pin the test-file comment's figure the same way; extend the depth test to refuse at 40 levels, key at 39, and key `nest(39)` and `nest(38)` apart. (M42 review O2, O6, O7.)
