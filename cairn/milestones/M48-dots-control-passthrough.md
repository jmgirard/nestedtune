...
- [x] T8: Review round 1, finding 1 (AC5): close the `call` hole in `check_dots_control()` — force the dots in the orchestrators (`check_dots_control(rlang::list2(...))`) or rename the formal `.call`; add a `call = ` refusal probe to both checks files.
- [x] T9: Review round 1, finding 2: an inline `tune::control_bayes()` in `...` draws its `seed` before `nested_loop()` snapshots `.Random.seed`; snapshot and restore around the forcing (or snapshot at orchestrator entry), with a test that `set.seed(s); nested_tune_bayes(..., control = tune::control_bayes())` leaves the stream restored and equals the no-control run.
- [x] T10: Review round 1, findings 3 and 7: correct the "Not returned" heading on both pages — the final fit keeps the inner `tune_results` in `$tuning`, so `extract`/`save_pred`/`save_workflow` do return there — and reword `workflow_size` against `save_workflow`.
- [x] T11: Review round 1, finding 4: a grid-path pass-through test (a `parallel_over = "everything"` probe on the stochastic grid fixture, or equivalent) showing a caller's `control_grid()` reaches the inner call.
- [x] T12: Review round 1, finding 5: `tune::control_last_fit()` carries the `control_grid` class in tune 2.1.0 and passes `check_control()`; refuse it or document it, and put it in the checks tests' class list either way.
...
**Gate dispositions (2026-09-02).** Findings 1, 2 and 4 rejected: 1 follows the convention `param_info`, `event_level` and `eval_time` already set on the same internal hops, and the stub case it cites was caught by the full suite; 2 is what AC4 specifies, the failure is loud, and the Bayes page's prose and every recipe say the recorded control carries no `seed` and set it first; 4 was rejected in round 1 with no new evidence. Finding 3 accepted as a limitation: the `time_limit` caveat is written as a DESIGN.md Known issues entry in the hygiene commit, IP2's text unchanged. Merge approved at the gate.
