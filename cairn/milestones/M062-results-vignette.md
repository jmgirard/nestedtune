...
- [x] T1: Draft the page: the guide's guard extended to `recipes`, design and run, then the anatomy section listing each column (`splits`, `id`, `.metrics`, `.selected`, `.inner_metrics`, `.notes`, `.completed`, the two seed columns) with one sentence and an executed peek.
