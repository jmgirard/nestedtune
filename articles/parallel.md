...
and the serial run below is why.
...
two results are identical, and which one was computed on daemons is not
recoverable from either. That is what lets a pool be a matter of the
session rather than of the analysis: a script gives the same answer on a
laptop with no daemons and on a workstation with many.
...
object sends that object with every fold. Building the workflow at the
top level avoids the second.
...
Here is that pool, with the warning caught by a handler and its class
printed. The results it returns are the same as before.
...
nd_res <- withCallingHandlers(
  nested_tune_grid(wf, folds, grid = grid),
  nestedtune_pool_not_cancellable = function(w) {
    caught <<- w
