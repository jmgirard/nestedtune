...
settle <- function(m, deadline = 15) {
  ...
  while (unresolved(m) && Sys.time() < end) Sys.sleep(0.05)
  if (unresolved(m)) return(structure(list(), class = "PROBE_TIMED_OUT"))
  m$data
}

describe <- function(label, x) {
  int <- suppressWarnings(tryCatch(as.integer(x), error = function(e) NA_integer_))
  named <- tryCatch(nanonext::nng_error(int), error = function(e) NA_character_)
  msg <- tryCatch(conditionMessage(x), error = function(e) "<conditionMessage() RAISES>")
  data.frame(
    probe          = label,
...
out <- list()
reset <- function() { daemons(0); Sys.sleep(0.3) }

# --- 1. stop_mirai() on a task actively running on a daemon ----------------
reset(); daemons(1)
m <- mirai({ Sys.sleep(30); "done" })
...
reset(); daemons(1)
busy <- mirai({ Sys.sleep(30); "busy" })
Sys.sleep(1.5)
queued <- mirai({ "never runs" })    # 1 daemon, so this waits
stop_mirai(queued)
...
reset(); daemons(1)
m <- mirai({ tools::pskill(Sys.getpid()); "unreachable" })
...
reset(); daemons(1)
mp <- mirai_map(1:2, function(i) { Sys.sleep(30); i })
Sys.sleep(1.5)
stop_mirai(mp)
collected <- tryCatch(collect_mirai(mp), error = function(e) list(e))
for (i in seq_along(collected)) {
  out[[length(out) + 1L]] <- describe(paste0("mirai_map stopped [", i, "]"), collected[[i]])
}
